...
// JSON body parsing middleware 추가 (안드로이드의 Gson과 유사한 역할)
...
중요 가이드라인:
- !!가장 중요 설명 작성 시 우선순위 1순위!! 감성적인 어투보다 재밌고 웃기고 약간 까는 맛이 있는 어투여야 함.
- !!가장 중요 설명 작성 시 우선순위 1순위!! 결과를 보고 친구들에게 웃기다며 공유할만한 설명이어야 함.
...
- detailed_interpretation: 감성적 해석 3~4개 + 100자 정도의 상세 설명 글 (캐주얼하고 친근한 톤으로)
    ...
    if (jsonStart !== -1 && jsonEnd !== -1) {
      try {
        ...
        analysis = {
          ...
          distance_evaluation: "꽤 가까워요! 하지만 충분히 갈 수 있어요 💪",
          ai_comment: "현재 프사도 충분히 매력적이야! 조금만 더 신경 써보면 추구미에 딱 맞을 것 같아",
...
// 1단계: 배경 이미지 프롬프트 생성 엔드포인트
app.post('/generate-background-prompts', upload.single('profile'), async (req, res) => {
  try {
    console.log('🎨 배경 프롬프트 생성 요청 받음');
    
    const { profileAnalysis, chugumiSummary } = req.body;
    
    if (!profileAnalysis || !chugumiSummary || !req.file) {
      return res.status(400).json({ 
        error: '프로필 분석 데이터, 추구미 요약, 프로필 사진이 모두 필요합니다' 
      });
    }
    ...
    // GPT-4 Vision으로 더 정교한 프롬프트 생성
    const promptText = `당신은 프로필 사진 배경 이미지 생성 전문가입니다. 
제공된 프로필 사진과 분석 데이터를 기반으로 DALL-E 3 이미지 생성에 최적화된 프롬프트를 생성해주세요.
...
**요구사항:**
1. 실사 스타일 프롬프트 1개 (사람 절대 나오지 않도록. 자연물 위주.)
2. 아티스틱 스타일 프롬프트 1개 (패턴, 그라데이션 등 기하학 무늬 류)

각 프롬프트는:
- !!가장 중요 우선순위 1순위!! 카카오톡 프로필 사진의 배경 사진으로 쓸만 해야 함.
- !!가장 중요 우선순위 1순위!! 사람은 절대 나오지 않도록.
- !!가장 중요 우선순위 1순위!! 프로필 사진의 색감와 뽑혀 나오는 배경 사진의 색감이 조화로워야 함.
- 사진 속 인물의 스타일, 색감, 분위기와 조화로운 배경
- 프로필 사진으로 사용하기 적합한 구도와 조명
- DALL-E 3에 최적화된 상세한 영어 프롬프트
- 1024x1024 고품질 이미지 생성에 적합

JSON 형태로 응답해주세요:
{
  "realistic_prompt": "실사 스타일 프롬프트",
  "artistic_prompt": "아티스틱 스타일 프롬프트", 
  "style_analysis": "스타일 분석 요약"
}`;
    ...
    console.log('OpenAI API 호출 중 (프롬프트 생성)...');
    const response = await openai.chat.completions.create(promptGenerationRequest);
    ...
    try {
      ...
      // Fallback 프롬프트 생성
      promptData = {
        realistic_prompt: "A professional photography background with soft natural lighting, complementing the style and mood visible in the reference image. Natural outdoor setting with gentle shadows, warm tones, photorealistic style, high quality, 1024x1024 resolution.",
        artistic_prompt: "An artistic painted-style background with dreamy atmosphere, soft brush strokes, warm color palette that harmonizes with the aesthetic shown in the reference image. Creative artistic mood, illustration style mixed with photography, 1024x1024 resolution.",
        style_analysis: "맞춤형 프롬프트를 생성했습니다."
      };
    }
...
app.post('/generate-backgrounds', async (req, res) => {
  try {
    console.log('🖼️ 배경 이미지 생성 요청 받음');
    
    const { realisticPrompt, artisticPrompt } = req.body;
    
    if (!realisticPrompt || !artisticPrompt) {
      return res.status(400).json({ 
        error: '실사 및 아티스틱 프롬프트가 모두 필요합니다' 
      });
    }

    const prompts = [
      { prompt: realisticPrompt, type: 'realistic', label: '🌅실사 느낌', style: 'natural' },
      { prompt: artisticPrompt, type: 'artistic', label: '🎨그림 느낌', style: 'vivid' }
    ];

    console.log('DALL-E 3 API 호출 시작...');
    
    const imagePromises = prompts.map(async ({ prompt, type, label, style }) => {
      try {
        ...
        console.error(`배경 이미지 ${type} 생성 오류:`, error);
        return null;
      }
    });
